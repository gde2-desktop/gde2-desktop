...
/* gobject signals */
static void gde2_color_button_finalize      (GObject             *object);
static void gde2_color_button_set_property  (GObject        *object,
					    guint           param_id,
					    const GValue   *value,
					    GParamSpec     *pspec);
static void gde2_color_button_get_property  (GObject        *object,
					    guint           param_id,
					    GValue         *value,
					    GParamSpec     *pspec);

/* gtkwidget signals */
static void gde2_color_button_state_changed (GtkWidget           *widget, 
					    GtkStateType         previous_state);

/* gtkbutton signals */
static void gde2_color_button_clicked       (GtkButton           *button);

/* source side drag signals */
static void gde2_color_button_drag_begin (GtkWidget        *widget,
					 GdkDragContext   *context,
					 gpointer          data);
static void gde2_color_button_drag_data_get (GtkWidget        *widget,
                                            GdkDragContext   *context,
...
gde2_color_button_class_init (Gde2ColorButtonClass *klass)
{
  ...
  gobject_class->get_property = gde2_color_button_get_property;
  gobject_class->set_property = gde2_color_button_set_property;
  gobject_class->finalize = gde2_color_button_finalize;
  widget_class->state_changed = gde2_color_button_state_changed;
  button_class->clicked = gde2_color_button_clicked;
   ...
   * When handling this signal, use gde2_color_button_get_color() and 
   * gde2_color_button_get_alpha() to find out which color was just selected.
   *
...
gde2_color_button_get_checkered (void)
{
...
#endif
{
  ...
  if (gde2_color_button_has_alpha (color_button))
    {
      cairo_save (cr);
      ...
      checkered = gde2_color_button_get_checkered ();
...
static void
gde2_color_button_state_changed (GtkWidget   *widget, 
                                GtkStateType previous_state)
{
...
static void
gde2_color_button_drag_data_received (GtkWidget        *widget,
				     GdkDragContext   *context,
				     gint              x,
				     gint              y,
				     GtkSelectionData *selection_data,
...
static void
gde2_color_button_drag_begin (GtkWidget      *widget,
			     GdkDragContext *context,
			     gpointer        data)
{
...
static void
gde2_color_button_drag_data_get (GtkWidget        *widget,
				GdkDragContext   *context,
				GtkSelectionData *selection_data,
				guint             info,
				guint             time,
...
gde2_color_button_init (Gde2ColorButton *color_button)
{
  ...
  /* Create the widgets */
  color_button->priv = GDE2_COLOR_BUTTON_GET_PRIVATE (color_button);
  ...
  g_signal_connect (color_button, "drag-begin",
		    G_CALLBACK (gde2_color_button_drag_begin), color_button);
  g_signal_connect (color_button, "drag-data-received",
                    G_CALLBACK (gde2_color_button_drag_data_received), color_button);
  g_signal_connect (color_button, "drag-data-get",
                    G_CALLBACK (gde2_color_button_drag_data_get), color_button);
...
gde2_color_button_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (gde2_color_button_parent_class)->finalize (object);
}


/**
 * gde2_color_button_new:
 *
 ...
 */
GtkWidget *
gde2_color_button_new (void)
{
  return g_object_new (GDE2_TYPE_COLOR_BUTTON, NULL);
}

/**
 * gde2_color_button_new_with_color:
 ...
 */
GtkWidget *
gde2_color_button_new_with_color (const GdkColor *color)
{
  return g_object_new (GDE2_TYPE_COLOR_BUTTON, "color", color, NULL);
}
		   ...
		   gpointer   data)
{
  ...
  gde2_color_selection_get_current_color (color_selection, &color_button->priv->color);
  color_button->priv->alpha = gde2_color_selection_get_current_alpha (color_selection);
...
gde2_color_button_clicked (GtkButton *button)
{
  ...
  if (!color_button->priv->cs_dialog) 
    {
      ...
      color_button->priv->cs_dialog = gde2_color_selection_dialog_new (color_button->priv->title);
      
      color_dialog = GDE2_COLOR_SELECTION_DIALOG (color_button->priv->cs_dialog);
  ...
  gde2_color_selection_set_has_opacity_control (GDE2_COLOR_SELECTION (color_dialog->colorsel),
                                               color_button->priv->use_alpha);

  gde2_color_selection_set_has_palette (GDE2_COLOR_SELECTION (color_dialog->colorsel), TRUE);
  
  gde2_color_selection_set_previous_color (GDE2_COLOR_SELECTION (color_dialog->colorsel), 
					  &color_button->priv->color);
  gde2_color_selection_set_previous_alpha (GDE2_COLOR_SELECTION (color_dialog->colorsel), 
					  color_button->priv->alpha);

  gde2_color_selection_set_current_color (GDE2_COLOR_SELECTION (color_dialog->colorsel), 
					 &color_button->priv->color);
  gde2_color_selection_set_current_alpha (GDE2_COLOR_SELECTION (color_dialog->colorsel), 
					 color_button->priv->alpha);
...
static void
gde2_color_button_set_property (GObject      *object,
			       guint         param_id,
			       const GValue *value,
			       GParamSpec   *pspec)
{
  ...
  switch (param_id) 
    {
    case PROP_USE_ALPHA:
      gde2_color_button_set_use_alpha (color_button, g_value_get_boolean (value));
      break;
    case PROP_TITLE:
      gde2_color_button_set_title (color_button, g_value_get_string (value));
      break;
    case PROP_COLOR:
      gde2_color_button_set_color (color_button, g_value_get_boxed (value));
      break;
    case PROP_ALPHA:
      gde2_color_button_set_alpha (color_button, g_value_get_uint (value));
      break;
    default:
...
static void
gde2_color_button_get_property (GObject    *object,
			       guint       param_id,
			       GValue     *value,
			       GParamSpec *pspec)
{
  ...
  switch (param_id) 
    {
    case PROP_USE_ALPHA:
      g_value_set_boolean (value, gde2_color_button_get_use_alpha (color_button));
      break;
    case PROP_TITLE:
      g_value_set_string (value, gde2_color_button_get_title (color_button));
      break;
    case PROP_COLOR:
      ...
      break;
    case PROP_ALPHA:
      g_value_set_uint (value, gde2_color_button_get_alpha (color_button));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, param_id, pspec);
      break;
