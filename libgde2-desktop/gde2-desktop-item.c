...
/* gde2-desktop-item.c - GDE2 Desktop File Representation
...
readbuf_open (GFile *file, GError **error)
{
	...
	if (stream == NULL) {
		g_set_error (error,
			     /* FIXME: better errors */
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_CANNOT_OPEN,
...
{
	if (!type)
		return GDE2_DESKTOP_ITEM_TYPE_NULL;

	switch (type [0]) {
	case 'A':
		if (!strcmp (type, "Application"))
			return GDE2_DESKTOP_ITEM_TYPE_APPLICATION;
		break;
	case 'L':
		if (!strcmp (type, "Link"))
			return GDE2_DESKTOP_ITEM_TYPE_LINK;
		break;
	case 'F':
		if (!strcmp (type, "FSDevice"))
			return GDE2_DESKTOP_ITEM_TYPE_FSDEVICE;
		break;
	case 'M':
		if (!strcmp (type, "MimeType"))
			return GDE2_DESKTOP_ITEM_TYPE_MIME_TYPE;
		break;
	case 'D':
		if (!strcmp (type, "Directory"))
			return GDE2_DESKTOP_ITEM_TYPE_DIRECTORY;
		break;
	case 'S':
		if (!strcmp (type, "Service"))
			return GDE2_DESKTOP_ITEM_TYPE_SERVICE;

		else if (!strcmp (type, "ServiceType"))
			return GDE2_DESKTOP_ITEM_TYPE_SERVICE_TYPE;
		break;
	default:
		break;
	}

	return GDE2_DESKTOP_ITEM_TYPE_OTHER;
}
...
gde2_desktop_item_new (void)
{
	...
	gde2_desktop_item_set_string (retval,
					...
					* a document */
				       _("No name"));
	gde2_desktop_item_set_string (retval,
				       GDE2_DESKTOP_ITEM_ENCODING,
				       "UTF-8");
	gde2_desktop_item_set_string (retval,
				       GDE2_DESKTOP_ITEM_VERSION,
				       "1.0");
...
read_sort_order (MateDesktopItem *item, GFile *dir)
{
	...
	readbuf_close (rb);
	if (str != NULL) {
		gde2_desktop_item_set_string (item, GDE2_DESKTOP_ITEM_SORT_ORDER,
					       str->str);
...
make_fake_directory (GFile *dir)
{
	...
	gde2_desktop_item_set_entry_type (item,
	...
	gde2_desktop_item_set_location_gfile (item, child);
	item->mtime = 0;
	g_object_unref (child);
				  ...
				  GError **error)
{
	...
	retval = gde2_desktop_item_new_from_gfile (gfile, flags, error);
				 ...
				 GError **error)
{
	...
	retval = gde2_desktop_item_new_from_gfile (file, flags, error);
	g_object_unref (file);
				   ...
				   GError **error)
{
	...
	if (type != G_FILE_TYPE_REGULAR && type != G_FILE_TYPE_DIRECTORY) {
		...
		g_set_error (error,
			     /* FIXME: better errors */
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_INVALID_TYPE,
	...
	if (type == G_FILE_TYPE_DIRECTORY) {
		...
		if (child_info == NULL) {
			...
			if (flags & GDE2_DESKTOP_ITEM_LOAD_ONLY_IF_EXISTS) {
				return NULL;
			} else {
	...
	retval = ditem_load (rb,
			     (flags & GDE2_DESKTOP_ITEM_LOAD_NO_TRANSLATIONS) != 0,
			     error);
	...
	if (flags & GDE2_DESKTOP_ITEM_LOAD_ONLY_IF_EXISTS &&
	    ! gde2_desktop_item_exists (retval)) {
		gde2_desktop_item_unref (retval);
		g_object_unref (subfn);
		return NULL;
	}

	retval->mtime = DONT_UPDATE_MTIME;
	gde2_desktop_item_set_location_gfile (retval, subfn);
	retval->mtime = mtime;
                                      ...
                                      GError **error)
{
	...
	retval = gde2_desktop_item_new_from_file (file, flags, error);
	g_free (file);
			 ...
			 GError **error)
{
	...
	if (uri == NULL) {
		g_set_error (error,
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_NO_FILENAME,
		  ...
		  AddedStatus             *added_status)
{
	...
	case 'm':
		...
		cs = gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_MINI_ICON);
		if (cs != NULL) {
	...
	case 'i':
		...
		cs = gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_ICON);
		if (cs != NULL) {
		...
		break;
	case 'c':
		cs = gde2_desktop_item_get_localestring (item, GDE2_DESKTOP_ITEM_NAME);
		if (cs != NULL) {
			esc = escape_single_quotes (cs, in_single_quotes, in_double_quotes);
			g_string_append (str, esc);
			g_free (esc);
	...
	case 'k':
		...
		break;
	case 'v':
		cs = gde2_desktop_item_get_localestring (item, GDE2_DESKTOP_ITEM_DEV);
		if (cs != NULL) {
			esc = escape_single_quotes (cs, in_single_quotes, in_double_quotes);
			g_string_append (str, esc);
			g_free (esc);
		     ...
		     SnLauncherContext *sn_context)
{
	...
	data = g_object_get_data (G_OBJECT (screen), "gde2-startup-data");
	if (data == NULL) {
		...
		g_object_set_data_full (G_OBJECT (screen), "gde2-startup-data",
	       ...
	       GError **error)
{
	...
	if (item->type == GDE2_DESKTOP_ITEM_TYPE_APPLICATION) {
		working_dir = gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_PATH);
		if (working_dir &&
	...
	if (gde2_desktop_item_get_boolean (item, GDE2_DESKTOP_ITEM_TERMINAL)) {
		const char *options =
			gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_TERMINAL_OPTIONS);
		...
		gde2_desktop_prepend_terminal_to_vector (&term_argc, &term_argv);
	}
	...
	startup_class = gde2_desktop_item_get_string (item,
						       "StartupWMClass");
	if (startup_class ||
	    gde2_desktop_item_get_boolean (item, "StartupNotify")) {
		const char *name;
		const char *icon;
		...
		name = gde2_desktop_item_get_localestring (item,
							    GDE2_DESKTOP_ITEM_NAME);

		if (name == NULL)
			name = gde2_desktop_item_get_localestring (item,
								    GDE2_DESKTOP_ITEM_GENERIC_NAME);
		...
		icon = gde2_desktop_item_get_string (item,
						      GDE2_DESKTOP_ITEM_ICON);
		...
		GError                      **error)
{
	...
	exec = gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_EXEC);
	/* This is a URL, so launch it as a url */
	if (item->type == GDE2_DESKTOP_ITEM_TYPE_LINK) {
		const char *url;
		gboolean    retval;
		...
		if (!(url && url[0] != '\0')) {
			g_set_error (error,
				     GDE2_DESKTOP_ITEM_ERROR,
				     GDE2_DESKTOP_ITEM_ERROR_NO_URL,
				     _("No URL to launch"));
			return -1;
		}
	...
	if (item->type != GDE2_DESKTOP_ITEM_TYPE_APPLICATION) {
		g_set_error (error,
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_NOT_LAUNCHABLE,
	...
	if (exec == NULL ||
		...
		g_set_error (error,
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_NO_EXEC_STRING,
	...
	if ( ! strip_the_amp (the_exec)) {
		g_set_error (error,
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_BAD_EXEC_STRING,
			     _("Bad command (Exec) to launch"));
		return -1;
	}

	ret = ditem_execute (item, the_exec, file_list, screen, workspace, envp,
			     (flags & GDE2_DESKTOP_ITEM_LAUNCH_ONLY_ONE),
			     (flags & GDE2_DESKTOP_ITEM_LAUNCH_USE_CURRENT_DIR),
			     (flags & GDE2_DESKTOP_ITEM_LAUNCH_APPEND_URIS),
			     (flags & GDE2_DESKTOP_ITEM_LAUNCH_APPEND_PATHS),
			     (flags & GDE2_DESKTOP_ITEM_LAUNCH_DO_NOT_REAP_CHILD),
			     error);
...
/**
 * gde2_desktop_item_launch_with_env:
 * @item: A desktop item
 * @file_list:  Files/URIs to launch this item with, can be %NULL
 * @flags: FIXME
 ...
 * @error: FIXME
 *
 * See gde2_desktop_item_launch for a full description. This function
...
/**
 * gde2_desktop_item_launch_on_screen:
 * @item: A desktop item
 * @file_list:  Files/URIs to launch this item with, can be %NULL
 * @flags: FIXME
...
/**
 * gde2_desktop_item_drop_uri_list:
 * @item: A desktop item
 ...
 *
 * Returns: The value returned by #gde2_execute_async() upon execution of
...
/**
* gde2_desktop_item_drop_uri_list_with_env:
* @item: A desktop item
* @uri_list: text as gotten from a text/uri-list
* @flags: FIXME
...
* @error: FIXME
*
* See gde2_desktop_item_drop_uri_list for a full description. This function
* additionally passes an environment vector for the child process
* which is to be launched.
*
* Returns: The value returned by #gde2_execute_async() upon execution of
					   ...
					   GError **error)
{
	...
	ret =  gde2_desktop_item_launch_with_env (
...
gde2_desktop_item_exists (const MateDesktopItem *item)
{
	...
	try_exec = lookup (item, GDE2_DESKTOP_ITEM_TRY_EXEC);
	...
	if (item->type == GDE2_DESKTOP_ITEM_TYPE_APPLICATION) {
		...
		exec = lookup (item, GDE2_DESKTOP_ITEM_EXEC);
		if (exec == NULL)
			return FALSE;
...
/**
 * gde2_desktop_item_get_entry_type:
 * @item: A desktop item
 *
				   ...
				   MateDesktopItemType type)
{
	...
	switch (type) {
	case GDE2_DESKTOP_ITEM_TYPE_NULL:
		set (item, GDE2_DESKTOP_ITEM_TYPE, NULL);
		break;
	case GDE2_DESKTOP_ITEM_TYPE_APPLICATION:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "Application");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_LINK:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "Link");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_FSDEVICE:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "FSDevice");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_MIME_TYPE:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "MimeType");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_DIRECTORY:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "Directory");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_SERVICE:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "Service");
		break;
	case GDE2_DESKTOP_ITEM_TYPE_SERVICE_TYPE:
		set (item, GDE2_DESKTOP_ITEM_TYPE, "ServiceType");
		break;
	default:
		break;
	}
...
/**
 * gde2_desktop_item_get_file_status:
 * @item: A desktop item
 *
...
gde2_desktop_item_get_file_status (const MateDesktopItem *item)
{
	...
	g_return_val_if_fail (item != NULL, GDE2_DESKTOP_ITEM_DISAPPEARED);
	g_return_val_if_fail (item->refcount > 0, GDE2_DESKTOP_ITEM_DISAPPEARED);

	if (item->location == NULL)
		return GDE2_DESKTOP_ITEM_DISAPPEARED;

	file = g_file_new_for_uri (item->location);
	info = g_file_query_info (file, G_FILE_ATTRIBUTE_TIME_MODIFIED,
				  G_FILE_QUERY_INFO_NONE, NULL, NULL);

	retval = GDE2_DESKTOP_ITEM_UNCHANGED;

	if (!g_file_info_has_attribute (info, G_FILE_ATTRIBUTE_TIME_MODIFIED))
		retval = GDE2_DESKTOP_ITEM_DISAPPEARED;
	else if (item->mtime < g_file_info_get_attribute_uint64 (info, G_FILE_ATTRIBUTE_TIME_MODIFIED))
		retval = GDE2_DESKTOP_ITEM_CHANGED;
 ...
 */
char *
gde2_desktop_item_find_icon (GtkIconTheme *icon_theme,
			      const char *icon,
			      int desired_size,
			      int flags)
{
			     ...
			     GtkIconTheme *icon_theme)
{
	...
	icon = gde2_desktop_item_get_string (item, GDE2_DESKTOP_ITEM_ICON);

	return gde2_desktop_item_find_icon (icon_theme, icon,
					     48 /* desired_size */,
					     0 /* flags */);
}

/**
 * gde2_desktop_item_get_location:
 * @item: A desktop item
 *
...
/**
 * gde2_desktop_item_set_location:
 * @item: A desktop item
...
/**
 * gde2_desktop_item_set_location_file:
 * @item: A desktop item
...
gde2_desktop_item_set_location_file (MateDesktopItem *item, const char *file)
{
	...
	if (file != NULL) {
		...
		gfile = g_file_new_for_path (file);
		gde2_desktop_item_set_location_gfile (item, gfile);
		g_object_unref (gfile);
	} else {
		gde2_desktop_item_set_location (item, NULL);
	}
}
...
gde2_desktop_item_set_location_gfile (MateDesktopItem *item, GFile *file)
{
	...
	if (file != NULL) {
		...
		uri = g_file_get_uri (file);
		gde2_desktop_item_set_location (item, uri);
		g_free (uri);
	} else {
		gde2_desktop_item_set_location (item, NULL);
	}
			       ...
			       const char *value)
{
	...
	if (strcmp (attr, GDE2_DESKTOP_ITEM_TYPE) == 0)
...
/**
 * gde2_desktop_item_get_string_locale:
 * @item: A desktop item
...
standard_is_boolean (const char * key)
{
	...
	if (bools == NULL) {
		...
		g_hash_table_insert (bools,
				     GDE2_DESKTOP_ITEM_NO_DISPLAY,
				     GDE2_DESKTOP_ITEM_NO_DISPLAY);
		g_hash_table_insert (bools,
				     GDE2_DESKTOP_ITEM_HIDDEN,
				     GDE2_DESKTOP_ITEM_HIDDEN);
		g_hash_table_insert (bools,
				     GDE2_DESKTOP_ITEM_TERMINAL,
				     GDE2_DESKTOP_ITEM_TERMINAL);
		g_hash_table_insert (bools,
				     GDE2_DESKTOP_ITEM_READ_ONLY,
				     GDE2_DESKTOP_ITEM_READ_ONLY);
	}
...
standard_is_strings (const char *key)
{
	...
	if (strings == NULL) {
		...
		g_hash_table_insert (strings,
				     GDE2_DESKTOP_ITEM_FILE_PATTERN,
				     GDE2_DESKTOP_ITEM_FILE_PATTERN);
		g_hash_table_insert (strings,
				     GDE2_DESKTOP_ITEM_ACTIONS,
				     GDE2_DESKTOP_ITEM_ACTIONS);
		g_hash_table_insert (strings,
				     GDE2_DESKTOP_ITEM_MIME_TYPE,
				     GDE2_DESKTOP_ITEM_MIME_TYPE);
		g_hash_table_insert (strings,
				     GDE2_DESKTOP_ITEM_PATTERNS,
				     GDE2_DESKTOP_ITEM_PATTERNS);
		g_hash_table_insert (strings,
				     GDE2_DESKTOP_ITEM_SORT_ORDER,
				     GDE2_DESKTOP_ITEM_SORT_ORDER);
	}
...
get_encoding (ReadBuf *rb)
{
	...
	while (readbuf_gets (buf, sizeof (buf), rb) != NULL) {
		if (strncmp (GDE2_DESKTOP_ITEM_ENCODING,
			     buf,
			     strlen (GDE2_DESKTOP_ITEM_ENCODING)) == 0) {
			char *p = &buf[strlen (GDE2_DESKTOP_ITEM_ENCODING)];
			if (*p == ' ')
				p++;
			if (*p != '=')
				continue;
	...
	if (rb->uri != NULL && strstr (rb->uri, "gde2/apps/") != NULL) {
		/* old gde2 */
		return ENCODING_LEGACY_MIXED;
	}
	    ...
	    gboolean no_translations)
{
	...
	if (cur_section == NULL &&
	    strcmp (key, GDE2_DESKTOP_ITEM_ENCODING) == 0) {
		k = g_strdup (key);
		val = g_strdup ("UTF-8");
	} else {
		...
		if (old_kde &&
		    cur_section == NULL &&
		    strcmp (key, GDE2_DESKTOP_ITEM_SORT_ORDER) == 0 &&
		    strchr (val, ';') == NULL) {
			int i;
			for (i = 0; val[i] != '\0'; i++) {
				if (val[i] == ',')
...
setup_type (MateDesktopItem *item, const char *uri)
{
	...
	if (type == NULL && uri != NULL) {
		...
		if (base != NULL &&
			...
			g_hash_table_replace (item->main_hash,
					      g_strdup (GDE2_DESKTOP_ITEM_TYPE),
					      g_strdup ("Directory"));
			item->keys = g_list_prepend
				(item->keys, g_strdup (GDE2_DESKTOP_ITEM_TYPE));
			item->type = GDE2_DESKTOP_ITEM_TYPE_DIRECTORY;
		} else {
			item->type = GDE2_DESKTOP_ITEM_TYPE_NULL;
		}
		g_free (base);
	} else {
...
sanitize (MateDesktopItem *item, const char *uri)
{
	...
	type = lookup (item, GDE2_DESKTOP_ITEM_TYPE);

	/* understand old gde2 style url exec thingies */
	if (type != NULL && strcmp (type, "URL") == 0) {
		const char *exec = lookup (item, GDE2_DESKTOP_ITEM_EXEC);
		set (item, GDE2_DESKTOP_ITEM_TYPE, "Link");
		if (exec != NULL) {
			/* Note, this must be in this order */
			set (item, GDE2_DESKTOP_ITEM_URL, exec);
			set (item, GDE2_DESKTOP_ITEM_EXEC, NULL);
		}
	}

	/* we make sure we have Name, Encoding and Version */
	if (lookup (item, GDE2_DESKTOP_ITEM_NAME) == NULL) {
		char *name = try_english_key (item, GDE2_DESKTOP_ITEM_NAME);
		/* If no name, use the basename */
		if (name == NULL && uri != NULL)
			name = g_path_get_basename (uri);
		/* If no uri either, use same default as gde2_desktop_item_new */
		if (name == NULL) {
		...
		g_hash_table_replace (item->main_hash,
				      g_strdup (GDE2_DESKTOP_ITEM_NAME),
				      name);
		item->keys = g_list_prepend
			(item->keys, g_strdup (GDE2_DESKTOP_ITEM_NAME));
	}
	if (lookup (item, GDE2_DESKTOP_ITEM_ENCODING) == NULL) {
		/* We store everything in UTF-8 so write that down */
		g_hash_table_replace (item->main_hash,
				      g_strdup (GDE2_DESKTOP_ITEM_ENCODING),
				      g_strdup ("UTF-8"));
		item->keys = g_list_prepend
			(item->keys, g_strdup (GDE2_DESKTOP_ITEM_ENCODING));
	}
	if (lookup (item, GDE2_DESKTOP_ITEM_VERSION) == NULL) {
		/* this is the version that we follow, so write it down */
		g_hash_table_replace (item->main_hash,
				      g_strdup (GDE2_DESKTOP_ITEM_VERSION),
				      g_strdup ("1.0"));
		item->keys = g_list_prepend
			(item->keys, g_strdup (GDE2_DESKTOP_ITEM_VERSION));
	}
}
	    ...
	    GError **error)
{
	...
	if (encoding == ENCODING_UNKNOWN) {
		...
		g_set_error (error,
			     GDE2_DESKTOP_ITEM_ERROR,
			     GDE2_DESKTOP_ITEM_ERROR_UNKNOWN_ENCODING,
...
static gpointer
_gde2_desktop_item_copy (gpointer boxed)
{
	return gde2_desktop_item_copy (boxed);
}

static void
_gde2_desktop_item_free (gpointer boxed)
{
	gde2_desktop_item_unref (boxed);
}

GType
gde2_desktop_item_get_type (void)
{
	static GType type = 0;
...
GQuark
gde2_desktop_item_error_quark (void)
{
	static GQuark q = 0;
	if (q == 0)
		q = g_quark_from_static_string ("gde2-desktop-item-error-quark");
