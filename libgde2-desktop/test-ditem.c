...
test_ditem (const char *file)
{
	...
	ditem = gde2_desktop_item_new_from_file (file,
						  GDE2_DESKTOP_ITEM_LOAD_ONLY_IF_EXISTS,
						  NULL);
	if (ditem == NULL) {
	...
	text = gde2_desktop_item_get_location (ditem);
	g_print ("LOCATION: |%s|\n", text);

	type = gde2_desktop_item_get_entry_type (ditem);
	g_print ("TYPE: |%u|\n", type);

	text = gde2_desktop_item_get_string
		(ditem, GDE2_DESKTOP_ITEM_TYPE);
	g_print ("TYPE(string): |%s|\n", text);

	text = gde2_desktop_item_get_string
		(ditem, GDE2_DESKTOP_ITEM_EXEC);
	g_print ("EXEC: |%s|\n", text);

	text = gde2_desktop_item_get_string
		(ditem, GDE2_DESKTOP_ITEM_ICON);
	g_print ("ICON: |%s|\n", text);

	text = gde2_desktop_item_get_localestring
		(ditem, GDE2_DESKTOP_ITEM_NAME);
	g_print ("NAME: |%s|\n", text);

	text = gde2_desktop_item_get_localestring_lang
		(ditem, GDE2_DESKTOP_ITEM_NAME,
		 "cs_CZ");
	...
	text = gde2_desktop_item_get_localestring
		(ditem, GDE2_DESKTOP_ITEM_COMMENT);
	g_print ("COMMENT: |%s|\n", text);

	g_print ("Setting Name[de]=Neu gestzt! (I have no idea what that means)\n");
	gde2_desktop_item_set_localestring
		(ditem,
	...
	g_print ("URI: %s\n", uri);
	gde2_desktop_item_save (ditem, uri, FALSE, NULL);
	g_free (uri);
}
...
launch_item (const char *file)
{
	...
	ret = gde2_desktop_item_launch (ditem, file_list, 0, NULL);
