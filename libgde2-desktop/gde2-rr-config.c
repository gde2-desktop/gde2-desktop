		      ...
		      GError             **err)
{
	...
	parser->output = g_object_new (GDE2_TYPE_RR_OUTPUT_INFO, NULL);
	...
	parser->configuration = g_object_new (GDE2_TYPE_RR_CONFIG, NULL);
		    ...
		    GError             **err)
{
    ...
    if (strcmp (name, "output") == 0)
    {
	/* If no rotation properties were set, just use GDE2_RR_ROTATION_0 */
	if (parser->output->priv->rotation == 0)
	    parser->output->priv->rotation = GDE2_RR_ROTATION_0;
	     ...
	     GError             **err)
{
	...
	{
	    parser->output->priv->rotation |= GDE2_RR_ROTATION_90;
	}
	else if (strcmp (text, "upside_down") == 0)
	{
	    parser->output->priv->rotation |= GDE2_RR_ROTATION_180;
	}
	else if (strcmp (text, "right") == 0)
	{
	    parser->output->priv->rotation |= GDE2_RR_ROTATION_270;
	}
    }
	...
	if (strcmp (text, "yes") == 0)
	{
	    parser->output->priv->rotation |= GDE2_RR_REFLECT_X;
	}
    }
	...
	if (strcmp (text, "yes") == 0)
	{
	    parser->output->priv->rotation |= GDE2_RR_REFLECT_Y;
	}
    }
...
gde2_rr_config_finalize (GObject *gobject)
{
    ...
    G_OBJECT_CLASS (gde2_rr_config_parent_class)->finalize (gobject);
}
...
gde2_rr_config_load_current (MateRRConfig *config, GError **error)
{
    ...
    g_return_val_if_fail (GDE2_IS_RR_CONFIG (config), FALSE);

    a = g_ptr_array_new ();
    rr_outputs = gde2_rr_screen_list_outputs (config->priv->screen);
	...
	if (!output->priv->connected)
	{
	    ...
	    output->priv->rate = -1;
	    output->priv->rotation = GDE2_RR_ROTATION_0;
	}
	else
	{
	    ...
	    if (gde2_rr_output_is_laptop (rr_output))
	    ...
	    crtc = gde2_rr_output_get_crtc (rr_output);
	    mode = crtc? gde2_rr_crtc_get_current_mode (crtc) : NULL;
	    
	    if (crtc && mode)
	    {
		output->priv->on = TRUE;
		
		gde2_rr_crtc_get_position (crtc, &output->priv->x, &output->priv->y);
		output->priv->width = gde2_rr_mode_get_width (mode);
		output->priv->height = gde2_rr_mode_get_height (mode);
		output->priv->rate = gde2_rr_mode_get_freq (mode);
		output->priv->rotation = gde2_rr_crtc_get_current_rotation (crtc);
	    ...
	    mode = gde2_rr_output_get_preferred_mode (rr_output);
	    ...
	    if (mode)
	    {
		output->priv->pref_width = gde2_rr_mode_get_width (mode);
		output->priv->pref_height = gde2_rr_mode_get_height (mode);
	    }
	    else
	    {
        ...
        output->priv->primary = gde2_rr_output_get_is_primary (rr_output);
    ...
    g_assert (gde2_rr_config_match (config, config));
...
gde2_rr_config_load_filename (MateRRConfig *result, const char *filename, GError **error)
{
    ...
    if (filename == NULL)
      filename = gde2_rr_config_get_intended_filename ();

    current = gde2_rr_config_new_current (result->priv->screen, error);
	...
	{
	    if (gde2_rr_config_match (configs[i], current))
	    {
		int j;
		GPtrArray *array;
	...
	if (!found)
	    g_set_error (error, GDE2_RR_ERROR, GDE2_RR_ERROR_NO_MATCHING_CONFIG,
...
gde2_rr_config_new_stored (MateRRScreen *screen, GError **error)
{
    ...
    success = gde2_rr_config_load_filename (self, filename, error);
...
gde2_rr_config_match (MateRRConfig *c1, MateRRConfig *c2)
{
    ...
    g_return_val_if_fail (GDE2_IS_RR_CONFIG (c2), FALSE);
...
char *
gde2_rr_config_get_backup_filename (void)
{
...
char *
gde2_rr_config_get_intended_filename (void)
{
    ensure_config_directory ();
...
get_rotation_name (MateRRRotation r)
{
	...
	return "normal";
    if (r & GDE2_RR_ROTATION_90)
	return "left";
    if (r & GDE2_RR_ROTATION_180)
	return "upside_down";
...
gde2_rr_config_ensure_primary (MateRRConfig *configuration)
{
        ...
        for (i = 0; priv->outputs[i] != NULL; ++i) {
                ...
                if (laptop == NULL
                    && _gde2_rr_output_name_is_laptop (info->priv->name)) {
                        laptop = info;
                }
        }
...
gde2_rr_config_save (MateRRConfig *configuration, GError **error)
{
	...
	{
	    if (!gde2_rr_config_match (configurations[i], configuration))
 ...
 * RANDR outputs.
 *
 * Typically, @filename is the result of gde2_rr_config_get_intended_filename() or
 * gde2_rr_config_get_backup_filename().
 *
 ...
 * have a domain of #GDE2_RR_ERROR and a corresponding error code.
 *
 * If the file in question is loaded successfully but the configuration cannot
 * be applied, the @error will have a domain of #GDE2_RR_ERROR.  Note that an
 * error code of #GDE2_RR_ERROR_NO_MATCHING_CONFIG is not a real error; it
...
gde2_rr_config_apply_from_filename_with_time (MateRRScreen *screen, const char *filename, guint32 timestamp, GError **error)
{
    ...
    my_error = NULL;
    if (!gde2_rr_screen_refresh (screen, &my_error)) {
	    if (my_error) {
    ...
    stored = g_object_new (GDE2_TYPE_RR_CONFIG, "screen", screen, NULL);

    if (gde2_rr_config_load_filename (stored, filename, error))
    {
	gboolean result;

	gde2_rr_config_ensure_primary (stored);
	result = gde2_rr_config_apply_with_time (stored, screen, timestamp, error);
			...
			GError          **error)
{
    ...
    crtc_id = gde2_rr_crtc_get_id (crtc);
    output_name = gde2_rr_output_get_name (output);

    if (!gde2_rr_crtc_can_drive_output (crtc, output))
    {
	g_set_error (error, GDE2_RR_ERROR, GDE2_RR_ERROR_CRTC_ASSIGNMENT,
		     _("CRTC %d cannot drive output %s"), crtc_id, output_name);
	return FALSE;
    }

    if (!gde2_rr_output_supports_mode (output, mode))
    {
	g_set_error (error, GDE2_RR_ERROR, GDE2_RR_ERROR_CRTC_ASSIGNMENT,
		     _("output %s does not support mode %dx%d@%dHz"),
		     output_name,
		     gde2_rr_mode_get_width (mode),
		     gde2_rr_mode_get_height (mode),
		     gde2_rr_mode_get_freq (mode));
	return FALSE;
    }

    if (!gde2_rr_crtc_supports_rotation (crtc, rotation))
    {
	g_set_error (error, GDE2_RR_ERROR, GDE2_RR_ERROR_CRTC_ASSIGNMENT,
	      ...
	      info->rotation == rotation))
	{
	    g_set_error (error, GDE2_RR_ERROR, GDE2_RR_ERROR_CRTC_ASSIGNMENT,
			 ...
			 output_name,
			 gde2_rr_mode_get_id (info->mode), gde2_rr_mode_get_id (mode),
			 info->x, info->y,
			 x, y,
		...
		gpointer data)
{
    ...
    if (!gde2_rr_crtc_set_config_with_time (crtc,
					     state->timestamp,
					     info->x, info->y,
					     info->mode,
					     info->rotation,
...
{
    if ((info->rotation & GDE2_RR_ROTATION_270)		||
	(info->rotation & GDE2_RR_ROTATION_90))
    {
	return TRUE;
    }
    return FALSE;
		   ...
		   GError **error)
{
	...
	for (pass = 0; pass < 2; ++pass)
	{
		...
		mode_width = gde2_rr_mode_get_width (mode);
		mode_height = gde2_rr_mode_get_height (mode);
		mode_freq = gde2_rr_mode_get_freq (mode);
		    ...
		    (pass == 1 || mode_freq == output->priv->rate))
		{
			...
			crtc_assignment_unassign (assignment, crtc, gde2_rr_output);
		    } else
			accumulate_error (accumulated_error, my_error);
		}
	    }
...
get_required_virtual_size (CrtcAssignment *assign, int *width, int *height)
{
    ...
    *width = *height = 1;
	...
	w = gde2_rr_mode_get_width (info->mode);
	h = gde2_rr_mode_get_height (info->mode);
...
crtc_assignment_apply (CrtcAssignment *assign, guint32 timestamp, GError **error)
{
	...
	if (mode)
	{
	    ...
	    {
		if (!gde2_rr_crtc_set_config_with_time (crtc, timestamp, 0, 0, NULL, GDE2_RR_ROTATION_0, NULL, 0, error))
		{
		    success = FALSE;
		    break;
		}
	...
	gde2_rr_screen_set_size (assign->screen, width, height, width_mm, height_mm);
    ...
    gde2_rr_screen_set_primary_output (assign->screen, assign->primary);
