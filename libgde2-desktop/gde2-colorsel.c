...
static void gde2_color_selection_set_property    (GObject                 *object,
					         ...
					         const GValue            *value,
					         GParamSpec              *pspec);
static void gde2_color_selection_get_property    (GObject                 *object,
					         guint                    prop_id,
					         GValue                  *value,
					         GParamSpec              *pspec);

static void gde2_color_selection_realize         (GtkWidget               *widget);
static void gde2_color_selection_unrealize       (GtkWidget               *widget);
static void gde2_color_selection_show_all        (GtkWidget               *widget);
...
gde2_color_selection_class_init (MateColorSelectionClass *klass)
{
  ...
  gobject_class->finalize = gde2_color_selection_finalize;
  gobject_class->set_property = gde2_color_selection_set_property;
  gobject_class->get_property = gde2_color_selection_get_property;

  gobject_class->dispose = gde2_color_selection_dispose;

  widget_class = GTK_WIDGET_CLASS (klass);
  widget_class->realize = gde2_color_selection_realize;
  widget_class->unrealize = gde2_color_selection_unrealize;
  widget_class->show_all = gde2_color_selection_show_all;
  widget_class->grab_broken_event = gde2_color_selection_grab_broken;
...
gde2_color_selection_init (MateColorSelection *colorsel)
{
  ...
  priv = colorsel->private_data = G_TYPE_INSTANCE_GET_PRIVATE (colorsel, GDE2_TYPE_COLOR_SELECTION, ColorSelectionPrivate);
  priv->changing = FALSE;
...
/* GObject methods */
static void
gde2_color_selection_finalize (GObject *object)
{
  G_OBJECT_CLASS (gde2_color_selection_parent_class)->finalize (object);
}

static void
gde2_color_selection_set_property (GObject         *object,
				  guint            prop_id,
				  const GValue    *value,
				  GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_HAS_OPACITY_CONTROL:
      gde2_color_selection_set_has_opacity_control (colorsel, 
						   g_value_get_boolean (value));
      break;
    case PROP_HAS_PALETTE:
      gde2_color_selection_set_has_palette (colorsel, 
					   g_value_get_boolean (value));
      break;
    case PROP_CURRENT_COLOR:
      gde2_color_selection_set_current_color (colorsel, g_value_get_boxed (value));
      break;
    case PROP_CURRENT_ALPHA:
      gde2_color_selection_set_current_alpha (colorsel, g_value_get_uint (value));
      break;
    default:
...
static void
gde2_color_selection_get_property (GObject     *object,
				  guint        prop_id,
				  GValue      *value,
				  GParamSpec  *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_HAS_OPACITY_CONTROL:
      g_value_set_boolean (value, gde2_color_selection_get_has_opacity_control (colorsel));
      break;
    case PROP_HAS_PALETTE:
      g_value_set_boolean (value, gde2_color_selection_get_has_palette (colorsel));
      break;
    case PROP_CURRENT_COLOR:
      gde2_color_selection_get_current_color (colorsel, &color);
      g_value_set_boxed (value, &color);
      break;
    case PROP_CURRENT_ALPHA:
      g_value_set_uint (value, gde2_color_selection_get_current_alpha (colorsel));
      break;
...
gde2_color_selection_dispose (GObject *object)
{
  ...
  G_OBJECT_CLASS (gde2_color_selection_parent_class)->dispose (object);
}
...
gde2_color_selection_realize (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (gde2_color_selection_parent_class)->realize (widget);
}
...
gde2_color_selection_unrealize (GtkWidget *widget)
{
  ...
  GTK_WIDGET_CLASS (gde2_color_selection_parent_class)->unrealize (widget);
}
 ...
 */
static void
gde2_color_selection_show_all (GtkWidget *widget)
{
  gtk_widget_show (widget);
}

static gboolean 
gde2_color_selection_grab_broken (GtkWidget          *widget,
				 GdkEventGrabBroken *event)
{
...
get_current_colors (MateColorSelection *colorsel)
{
  ...
  gde2_color_selection_palette_from_string (DEFAULT_COLOR_PALETTE,
                                            &colors,
                                            &n_colors);
                      ...
                      gdouble           *color)
{
 ...
 out:
  ...
  if (change_palette_hook != default_change_palette_func)
	...
	g_warning ("gde2_color_selection_set_change_palette_hook used by widget is not on the default screen.");
                     ...
                     gpointer   data)
{
  ...
  colorsel = GDE2_COLOR_SELECTION (g_object_get_data (G_OBJECT (drawing_area),
                                                     "gtk-color-sel"));
      ...
      if (GPOINTER_TO_INT (g_object_get_data (G_OBJECT (widget), "color_set")) != 0)
        {
          ...
          set_color_internal (GDE2_COLOR_SELECTION (data), color);
        }
      return TRUE;
    }
...
shutdown_eyedropper (GtkWidget *widget)
{
  ...
  colorsel = GDE2_COLOR_SELECTION (widget);
  priv = colorsel->private_data;    
		    ...
		    gpointer       data)
{
  ...
  colorsel = GDE2_COLOR_SELECTION (g_object_get_data (G_OBJECT (adjustment), "COLORSEL"));
  priv = colorsel->private_data;
...
update_palette (MateColorSelection *colorsel)
{
  ...
  for (i = 0; i < GTK_CUSTOM_PALETTE_HEIGHT; i++)
      ...
      for (j = 0; j < GTK_CUSTOM_PALETTE_WIDTH; j++)
	{
          ...
          gde2_color_selection_set_palette_color (colorsel,
                                                 index,
...
palette_change_notify_instance (GObject    *object,
                                ...
                                gpointer    data)
{
  update_palette (GDE2_COLOR_SELECTION (data));
}
                             ...
                             gint            n_colors)
{
  ...
  str = gde2_color_selection_palette_to_string (colors, n_colors);

  gtk_settings_set_string_property (gtk_settings_get_for_screen (screen),
                                    "gtk-color-palette",
                                    str,
                                    "gde2_color_selection_palette_to_string");
...
gde2_color_selection_new (void)
{
  ...
  colorsel = g_object_new (GDE2_TYPE_COLOR_SELECTION, "orientation", GTK_ORIENTATION_VERTICAL, NULL);
  priv = colorsel->private_data;
  set_color_internal (colorsel, color);
  gde2_color_selection_set_has_opacity_control (colorsel, TRUE);
...
gde2_color_selection_get_has_opacity_control (MateColorSelection *colorsel)
{
  ...
  g_return_val_if_fail (GDE2_IS_COLOR_SELECTION (colorsel), FALSE);
...
gde2_color_selection_get_current_alpha (MateColorSelection *colorsel)
{
  ...
  g_return_val_if_fail (GDE2_IS_COLOR_SELECTION (colorsel), 0);
 ...
 * Calling gde2_color_selection_set_current_color() will also set this color the first
 * time it is called.
 **/
void
...
/**
 * gde2_color_selection_palette_from_string:
 ...
 **/
gboolean
gde2_color_selection_palette_from_string (const gchar *str,
                                         GdkColor   **colors,
                                         gint        *n_colors)
{
  GdkColor *retval;
 ...
 **/
gchar*
gde2_color_selection_palette_to_string (const GdkColor *colors,
                                       gint            n_colors)
{
  gint i;
  gchar **strs = NULL;
...
/**
 * gde2_color_selection_set_change_palette_hook:
...
/**
 * gde2_color_selection_set_change_palette_with_screen_hook:
 * @func: a function to call when the custom palette needs saving.
 * 
 * Installs a global function to be called whenever the user tries to
 * modify the palette in a color selection. This function should save
