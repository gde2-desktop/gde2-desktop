/*
 * gde2-thumbnail.c: Utilities for handling thumbnails
 *
				   ...
				   gboolean    preserve_aspect_ratio)
{
	...
	g_object_set_data (G_OBJECT (pixbuf), "gde2-original-width",
			   GINT_TO_POINTER (info.input_width));
	g_object_set_data (G_OBJECT (pixbuf), "gde2-original-height",
  ...
  if (!found)
    {
      ...
      if (thumb)
        gde2_desktop_thumbnail_factory_add_thumbnailer (factory, thumb);
    }
                                               ...
                                               Gde2DesktopThumbnailFactory *factory)
{
  ...
  if (priv->disabled)
  ...
  else
    {
      ...
      gde2_desktop_thumbnail_factory_load_thumbnailers (factory);
    }
...
gde2_desktop_thumbnail_factory_init (Gde2DesktopThumbnailFactory *factory)
{
  ...
  priv->size = GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL;
  ...
  priv->settings = g_settings_new ("org.gde2.thumbnailers");
					...
					time_t                 mtime)
{
  ...
  path = g_build_filename (g_get_user_cache_dir (),
                           "thumbnails",
                           (priv->size == GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
                           file,
                           NULL);
  g_free (file);
  ...
  if (pixbuf != NULL)
    {
      res = gde2_desktop_thumbnail_is_valid (pixbuf, uri, mtime);
					       ...
					       time_t                 mtime)
{
  ...
  if (!gde2_desktop_thumbnail_factory_is_disabled (factory, mime_type))
    {
      Thumbnailer *thumb;
  ...
  if (have_script || mimetype_supported_by_gdk_pixbuf (mime_type))
    {
      return !gde2_desktop_thumbnail_factory_has_valid_failed_thumbnail (factory,
                                                                          uri,
                                                                          mtime);
    }
						    ...
						    const char            *mime_type)
{
  ...
  size = 128;
  if (factory->priv->size == GDE2_DESKTOP_THUMBNAIL_SIZE_LARGE)
    size = 256;
  ...
  if (script)
    {
      ...
      fd = g_file_open_tmp (".gde2_desktop_thumbnail.XXXXXX", &tmpname, NULL);
  ...
  if (width > size || height > size)
    {
      ...
      scaled = gde2_desktop_thumbnail_scale_down_pixbuf (pixbuf,
...
make_thumbnail_dirs (Gde2DesktopThumbnailFactory *factory)
{
  ...
  image_dir = g_build_filename (thumbnail_dir,
				(factory->priv->size == GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
				NULL);
						...
						time_t                 original_mtime)
{
  ...
  if (tmp_fd == -1)
    {
      gde2_desktop_thumbnail_factory_create_failed_thumbnail (factory, uri, original_mtime);
      g_free (tmp_path);
      g_free (path);
      return;
    }
  ...
  if (width != NULL && height != NULL) 
    saved_ok  = gdk_pixbuf_save (thumbnail,
				 ...
				 "tEXt::Software", "GDE2::ThumbnailFactory",
				 NULL);
  else
    saved_ok  = gdk_pixbuf_save (thumbnail,
				 tmp_path,
 ...
 **/
char *
gde2_desktop_thumbnail_md5 (const char *uri)
{
...
/**
 * gde2_desktop_thumbnail_path_for_uri:
 * @uri: an uri
				      ...
				      Gde2DesktopThumbnailSize  size)
{
  ...
  md5 = gde2_desktop_thumbnail_md5 (uri);
  file = g_strconcat (md5, ".png", NULL);
  g_free (md5);
  
  path = g_build_filename (g_get_user_cache_dir (),
                           "thumbnails",
                           (size == GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
                           file,
                           NULL);
  g_free (file);
...
/**
 * gde2_desktop_thumbnail_has_uri:
 ...
 **/
gboolean
gde2_desktop_thumbnail_has_uri (GdkPixbuf          *pixbuf,
				 const char         *uri)
{
...
/**
 * gde2_desktop_thumbnail_is_valid:
 ...
 **/
gboolean
gde2_desktop_thumbnail_is_valid (GdkPixbuf          *pixbuf,
				  const char         *uri,
				  time_t              mtime)
{
