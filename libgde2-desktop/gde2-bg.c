...
gde2bg.c: Object for the desktop background.
...
#define GDE2_BG_CACHE_DIR "gde2/background"
...
gde2_bg_load_from_preferences (Gde2BG *bg)
{
	...
	gde2_bg_load_from_gsettings (bg, settings);
...
gde2_bg_load_from_system_preferences (Gde2BG *bg)
{
	...
	*/
	...
	gde2_bg_load_from_system_gsettings (bg, settings, FALSE);
			     ...
			     GSettings *settings)
{
	...
	bg->is_enabled = g_settings_get_boolean (settings, GDE2_BG_KEY_DRAW_BACKGROUND);

	/* Filename */
	filename = NULL;
	tmp = g_settings_get_string (settings, GDE2_BG_KEY_PICTURE_FILENAME);
	if (tmp && *tmp != '\0') {
		...
		if (filename != NULL && !g_file_test (filename, G_FILE_TEST_EXISTS)) {
			...
			g_settings_delay (settings);
			g_settings_reset (settings, GDE2_BG_KEY_PICTURE_FILENAME);
			filename = g_settings_get_string (settings, GDE2_BG_KEY_PICTURE_FILENAME);
			g_settings_revert (settings);
	...
	/* Colors */
	tmp = g_settings_get_string (settings, GDE2_BG_KEY_PRIMARY_COLOR);
	color_from_string (tmp, &c1);
	g_free (tmp);

	tmp = g_settings_get_string (settings, GDE2_BG_KEY_SECONDARY_COLOR);
	color_from_string (tmp, &c2);
	g_free (tmp);

	/* Color type */
	ctype = g_settings_get_enum (settings, GDE2_BG_KEY_COLOR_TYPE);

	/* Placement */
	placement = g_settings_get_enum (settings, GDE2_BG_KEY_PICTURE_PLACEMENT);

	gde2_bg_set_color (bg, ctype, &c1, &c2);
	gde2_bg_set_placement (bg, placement);
	gde2_bg_set_filename (bg, filename);
...
gde2_bg_save_to_preferences (Gde2BG *bg)
{
	...
	gde2_bg_save_to_gsettings (bg, settings);
	g_object_unref (settings);
}
			   ...
			   GSettings *settings)
{
	...
	g_settings_set_boolean (settings, GDE2_BG_KEY_DRAW_BACKGROUND, bg->is_enabled);
	g_settings_set_string (settings, GDE2_BG_KEY_PICTURE_FILENAME, bg->filename);
	g_settings_set_enum (settings, GDE2_BG_KEY_PICTURE_PLACEMENT, bg->placement);
	g_settings_set_string (settings, GDE2_BG_KEY_PRIMARY_COLOR, primary);
	g_settings_set_string (settings, GDE2_BG_KEY_SECONDARY_COLOR, secondary);
	g_settings_set_enum (settings, GDE2_BG_KEY_COLOR_TYPE, bg->color_type);
...
gde2_bg_dispose (GObject *object)
{
	...
	G_OBJECT_CLASS (gde2_bg_parent_class)->dispose (object);
}
...
gde2_bg_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (gde2_bg_parent_class)->finalize (object);
}
...
gde2_bg_class_init (Gde2BGClass *klass)
{
	...
	object_class->dispose = gde2_bg_dispose;
	object_class->finalize = gde2_bg_finalize;
...
{
	return g_build_filename (g_get_user_cache_dir(), GDE2_BG_CACHE_DIR, NULL);
}
		   ...
		   int *w, int *h)
{
	...
	switch (placement) {
	case GDE2_BG_PLACEMENT_SPANNED:
                new = pixbuf_scale_to_fit (pixbuf, width, height);
		break;
	case GDE2_BG_PLACEMENT_ZOOMED:
		new = pixbuf_scale_to_min (pixbuf, width, height);
		break;

	case GDE2_BG_PLACEMENT_FILL_SCREEN:
		new = gdk_pixbuf_scale_simple (pixbuf, width, height,
					       GDK_INTERP_BILINEAR);
		break;

	case GDE2_BG_PLACEMENT_SCALED:
		new = pixbuf_scale_to_fit (pixbuf, width, height);
		break;

	case GDE2_BG_PLACEMENT_CENTERED:
	case GDE2_BG_PLACEMENT_TILED:
	default:
	       ...
	       gboolean   is_root)
{
	...
	if (is_root && (bg->placement != GDE2_BG_PLACEMENT_SPANNED)) {
			...
			gboolean     root)
{
	...
	gde2_bg_get_pixmap_size (bg, width, height, &pm_width, &pm_height);
...
#endif
	if (!bg->filename && bg->color_type == GDE2_BG_COLOR_SOLID) {
#if GTK_CHECK_VERSION (3, 0, 0)
	...
	else
	{
		...
		pixbuf = gdk_pixbuf_new (GDK_COLORSPACE_RGB, FALSE, 8,
					 width, height);
		gde2_bg_draw (bg, pixbuf, gdk_window_get_screen (window), root);
		  ...
		  int      height)
{
	...
	if (bg->color_type == GDE2_BG_COLOR_SOLID) {
		color = bg->primary;
	} else {
...
#if GTK_CHECK_VERSION (3, 0, 0)
cairo_surface_t *
gde2_bg_get_surface_from_root (GdkScreen *screen)
#else
GdkPixmap *
gde2_bg_get_pixmap_from_root (GdkScreen *screen)
#endif
{
	int result;
	gint format;
 ...
 */
static void
gde2_bg_set_root_pixmap_id (GdkScreen       *screen,
			    Display         *display,
			    Pixmap           xpixmap)
{
 ...
 * to gde2_bg_create_surface().
 **/
void
#if GTK_CHECK_VERSION (3, 0, 0)
gde2_bg_set_surface_as_root (GdkScreen *screen, cairo_surface_t *surface)
#else
gde2_bg_set_pixmap_as_root  (GdkScreen *screen, GdkPixmap *pixmap)
#endif
{
...
#  endif
	...
	XGrabServer (display);
	gde2_bg_set_root_pixmap_id (screen, display, pixmap_id);
...
/**
 * gde2_bg_set_surface_as_root_with_crossfade:
 * @screen: the #GdkScreen to change root background on
 * @surface: the cairo xlib surface to set root background from
 *
 * Set the root pixmap, and properties pointing to it.
 * This function differs from gde2_bg_set_surface_as_root()
...
gde2_bg_set_surface_as_root_with_crossfade (GdkScreen       *screen,
		 			    cairo_surface_t *surface)
#else
gde2_bg_set_pixmap_as_root_with_crossfade (GdkScreen *screen,
		 			   GdkPixmap *pixmap)
#endif
{
	...
	fade        = gde2_bg_crossfade_new (width, height);
#if GTK_CHECK_VERSION (3, 0, 0)
	old_surface = gde2_bg_get_surface_from_root (screen);

	gde2_bg_crossfade_set_start_surface (fade, old_surface);
	gde2_bg_crossfade_set_end_surface (fade, surface);
#else
	old_pixmap = gde2_bg_get_pixmap_from_root (screen);

	gde2_bg_crossfade_set_start_pixmap (fade, old_pixmap);
	gde2_bg_crossfade_set_end_pixmap (fade, pixmap);
#endif
	...
	gde2_bg_set_surface_as_root (screen, surface);
#else
	g_object_unref (old_pixmap);

	gde2_bg_set_pixmap_as_root (screen, pixmap);
#endif
	gde2_bg_crossfade_start (fade, root_window);
	...
	if ((ent = file_cache_lookup (bg, PIXBUF, filename))) {
		...
		if (!pixbuf) {
			...
			if (g_strcmp0 (tmp, "svg") == 0 &&
			    ...
			    (bg->placement == GDE2_BG_PLACEMENT_FILL_SCREEN ||
			     bg->placement == GDE2_BG_PLACEMENT_SCALED ||
			     bg->placement == GDE2_BG_PLACEMENT_ZOOMED))
			{
		 ...
		 int	    dest_height)
{
	...
	if (placement != GDE2_BG_PLACEMENT_TILED &&
	    placement != GDE2_BG_PLACEMENT_CENTERED) {
			       ...
			       const char            *filename)
{
	...
	thumb = gde2_desktop_thumbnail_factory_lookup (factory, uri, mtime);
	...
	if (thumb) {
		...
		if (orig) {
			...
			g_object_set_data_full (G_OBJECT (result), "gde2-thumbnail-height",
						g_strdup_printf ("%d", orig_height), g_free);
			g_object_set_data_full (G_OBJECT (result), "gde2-thumbnail-width",
						g_strdup_printf ("%d", orig_width), g_free);

			g_object_unref (orig);

			gde2_desktop_thumbnail_factory_save_thumbnail (factory, result, uri, mtime);
		}
		else {
			gde2_desktop_thumbnail_factory_create_failed_thumbnail (factory, uri, mtime);
		}
	}
...
/**
 * gde2_bg_create_frame_thumbnail:
 *
