...
/* gde2-desktop-utils.c - Utilities for the GDE2 Desktop
...
/**
 * gde2_desktop_prepend_terminal_to_vector:
 * @argc: a pointer to the vector size
 * @argv: a pointer to the vector
 *
 * Prepends a terminal (either the one configured as default in the user's
 * GDE2 setup, or one of the common xterm emulators) to the passed in vector,
 ...
 * allocated separately.  That is the standard glib/GDE2 way of doing vectors
 ...
 **/
void
gde2_desktop_prepend_terminal_to_vector (int *argc, char ***argv)
{
        char **real_argv;
        int real_argc;
        int i, j;
	...
	if (term_argv == NULL) {
		...
		check = g_find_program_in_path ("gde2-terminal");
		if (check != NULL) {
			term_argv[0] = check;
			/* Note that gde2-terminal takes -x and
			 * as -e in gde2-terminal is broken we use that. */
...
/**
 * gde2_gdk_spawn_command_line_on_screen:
 * @screen: a GdkScreen
 ...
 **/
gboolean
gde2_gdk_spawn_command_line_on_screen (GdkScreen *screen, const gchar *command, GError **error)
{
