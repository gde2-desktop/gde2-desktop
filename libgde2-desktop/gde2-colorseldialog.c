...
static GObject * gde2_color_selection_dialog_buildable_get_internal_child (GtkBuildable *buildable,
									  GtkBuilder   *builder,
...
static void
gde2_color_selection_dialog_get_property (GObject         *object,
					 guint            prop_id,
					 GValue          *value,
					 GParamSpec      *pspec)
{
...
gde2_color_selection_dialog_init (Gde2ColorSelectionDialog *colorseldiag)
{
  ...
  colorseldiag->colorsel = gde2_color_selection_new ();
  gtk_container_set_border_width (GTK_CONTAINER (colorseldiag->colorsel), 5);
  gde2_color_selection_set_has_palette (GDE2_COLOR_SELECTION(colorseldiag->colorsel), FALSE); 
  gde2_color_selection_set_has_opacity_control (GDE2_COLOR_SELECTION(colorseldiag->colorsel), FALSE);
...
static void
gde2_color_selection_dialog_buildable_interface_init (GtkBuildableIface *iface)
{
  parent_buildable_iface = g_type_interface_peek_parent (iface);
  iface->get_internal_child = gde2_color_selection_dialog_buildable_get_internal_child;
}

static GObject *
gde2_color_selection_dialog_buildable_get_internal_child (GtkBuildable *buildable,
							 GtkBuilder   *builder,
							 const gchar  *childname)
{
    if (strcmp(childname, "ok_button") == 0)
	return G_OBJECT (GDE2_COLOR_SELECTION_DIALOG (buildable)->ok_button);
    else if (strcmp(childname, "cancel_button") == 0)
	return G_OBJECT (GDE2_COLOR_SELECTION_DIALOG (buildable)->cancel_button);
    else if (strcmp(childname, "help_button") == 0)
	return G_OBJECT (GDE2_COLOR_SELECTION_DIALOG(buildable)->help_button);
    else if (strcmp(childname, "color_selection") == 0)
	return G_OBJECT (GDE2_COLOR_SELECTION_DIALOG(buildable)->colorsel);
