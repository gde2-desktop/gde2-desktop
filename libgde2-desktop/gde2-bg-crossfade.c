/* gde2-bg-crossfade.h - fade window background between two surfaces
 *
...
static void
gde2_bg_crossfade_get_property (GObject    *object,
			     guint       property_id,
			     GValue     *value,
			     GParamSpec *pspec)
{
...
gde2_bg_crossfade_class_init (Gde2BGCrossfadeClass *fade_class)
{
	...
	gobject_class->get_property = gde2_bg_crossfade_get_property;
	gobject_class->set_property = gde2_bg_crossfade_set_property;
...
Gde2BGCrossfade* gde2_bg_crossfade_new (int width, int height)
{
	...
	object = g_object_new(GDE2_TYPE_BG_CROSSFADE,
		"width", width,
		"height", height,
		NULL);
 ...
 * gde2_bg_crossfade_set_start_surface() and
 * gde2_bg_crossfade_set_end_surface() need to be called. If animations
                         ...
                         GdkWindow        *window)
{
	...
	g_return_if_fail (!gde2_bg_crossfade_is_started (fade));
                                ...
                                GtkWidget       *widget)
{
	...
	gde2_bg_crossfade_start (fade, window);
}
...
gde2_bg_crossfade_stop (Gde2BGCrossfade *fade)
{
	...
	if (!gde2_bg_crossfade_is_started (fade))
		return;
